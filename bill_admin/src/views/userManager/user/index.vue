  ...
  <div>
    <Card>
      <Row :gutter="24" style="margin-bottom: 12px;">
        <Col span="6">
        <input-search v-model:value="userParams.userName" :placeholder="t('routes.user.header.placeholder')" enter-button
          @search="handleSearch" @change="handleSearch" />
        </Col>
        <Col span="6">
        <Button type="primary" @click="handleOpenAddOrEdit()">
          <template #icon><plus-outlined /></template>
          {{ t('routes.user.header.addBtn') }}
        </Button></Col>
      </Row>
      <Table :row-key="(record) => record.id" :dataSource="usersList" :columns="columns" bordered :loading="loading"
        :row-class-name="(_record, index) => (index % 2 === 1 ? 'table-striped' : null)" :pagination="{
          ...
          showTotal: (total) => `${t('routes.user.th.total')} ${total} ${t('routes.user.th.total1')}`,
        ...
        <template #state="{ record, column }">
          <template v-if="column.dataIndex === 'state'">
            <Switch v-model:checked="record.state" @change="handleUserState(record)"></Switch>
          </template>
        </template>
        ...
        <template #operation="{ column, record }">
          <template v-if="column.dataIndex === 'operation'">
            <HindButton :title="t('routes.user.tip.updateUser')" :opt="editOpt" @click="handleOpenAddOrEdit(record.id)">
              <template v-slot:icon>
                <EditOutlined />
              </template>
            </HindButton>
            <popconfirm :title="t('routes.user.tip.isDel')" :ok-text="t('routes.user.yes')"
              :cancel-text="t('routes.user.no')" @confirm="confirmDelete(record.id)" @cancel="cancelDelete">
              <HindButton :opt="deleteOpt" :title="t('routes.user.tip.delUser')" :danger="true">
                <template v-slot:icon>
                  <DeleteOutlined />
                </template>
              </HindButton>
            </popconfirm>
            <HindButton :title="t('routes.user.tip.assignRoles')" :opt="setOpt" @click="handleClick">
              <template v-slot:icon>
                <SettingOutlined />
              </template>
            </HindButton>
...
let checkUserName = async (_rule: RuleObject, value: string) => {
  if (!value) {
    return Promise.reject(`${t('routes.user.header.placeholder')}`);
  }
  if (value.length > 15 || value.length < 3) {
    return Promise.reject(`${t('routes.user.tip.userLen')}`);
  } else {
...
let checkUserPass = async (_rule: RuleObject, value: string) => {
  if (!value) {
    return Promise.reject(`${t('routes.user.tip.inputPass')}`);
  }
  if (value.length > 15 || value.length < 6) {
    return Promise.reject(`${t('routes.user.tip.passLen')}`);
  } else {
    return Promise.resolve();
  }
};
let checkEmail = async (_rule: RuleObject, value: string) => {
  ...
  if (!reg.test(value)) {
    return Promise.reject(`${t('routes.user.tip.emailErr')}`);
  } else {
    return Promise.resolve();
  }
};
let checkMobile = async (_rule: RuleObject, value: string) => {
  ...
  if (!value) return Promise.resolve();
  if (!reg.test(value)) {
    return Promise.reject(`${t('routes.user.tip.phoneErr')}`);
  } else {
    return Promise.resolve();
  }
};
...
const handleOpenAddOrEdit = async (id?: number) => {
  if (id) {
    ...
    try {
      ...
      message.error(`${t('routes.user.tip.serchErr')}`);
    }
  } else {
    dialogTitle.value = t('routes.user.header.addBtn');
  }
...
const handleAddOrEditOk = async () => {
  ...
  try {
    ...
    message.error(`${t('routes.user.tip.userInfoErr')}`);
  }
};
const handleCancel = () => {
  addOrEditUserDialog.value = false;
