...
// 通过验证模块获取分类管理
var rentServ = authorization.getService("RentService");
...
router.get(
  ...
  function (req, res, next) {
    ...
    if (req.query.dateTime) {
      conditions["dateTime"] = req.query.dateTime || new Date().getFullYear();
    }
    rentServ.getAllRent(conditions, function (err, result) {
...
router.post(
  ...
  // 参数验证
  function (req, res, next) {
    if (!req.body.cateId) return res.sendResult(null, 400, "请输入分类Id");
