 ...
 * @param  {Function}
 */
module.exports.getAllRent = function (params, cb) {
  var conditions = {};
  if (!params.pageNum || params.pageNum <= 0) return cb("pageNum 参数错误");
  conditions["columns"] = {};
  if (params.dateTime) {
    conditions["columns"]["rent_time"] = orm.like("%" + params.dateTime + "%");
  }
...
module.exports.addCategory = function (params, cb) {
  dao.create(
    ...
    function (err, newCat) {
      if (err) {
        if (err.errno == 1062) return cb("该分类名称已存在");
        return cb("创建分类失败");
      }
      cb(null, null);
    }
...
module.exports.updateCategory = function (cateId, params, cb) {
  dao.exists(
    ...
    function (err, res) {
      ...
      dao.update("CategoryModel", cateId, conditions, function (err, newCat) {
        if (err) return cb("更新失败");
        cb(null, null);
      });
    }
...
module.exports.deleteCategory = function (cateId, cb) {
  dao.update("CategoryModel", cateId, { deleted: 0 }, function (err, newCat) {
