  ...
  <div>
    ...
    -->
      <el-table border style="margin-bottom: 20px;" :data="menuPermissionList" :expand-row-keys="expandKeys" row-key="id">
        ...
        <el-table-column prop="toCode" label="路由地址" />
        <el-table-column label="操作">
          <template slot-scope="{row}">

            <HintButton :disabled="row.level===4" type="primary" icon="el-icon-plus" size="mini" @click="toAddPermission(row)" :title="getAddTitle(row.level)" v-if="$hasBtnPower('btn.Permission.add')" />

            <HintButton type="primary" icon="el-icon-edit" size="mini" :disabled="row.level===1" @click="toUpdatePermission(row)" :title="row.level===4 ? '修改功能' : '修改菜单'" v-if="$hasBtnPower('btn.Permission.update')" />

            <HintButton :disabled="row.level===1" type="danger" icon="el-icon-delete" size="mini" @click="removePermission(row)" title="删除" v-if="$hasBtnPower('btn.Permission.remove')" />
          </template>
        </el-table-column>
      </el-table>
